 ...
 *  completamente diversi (un tour leader vede solo i suoi bus). Un tour
 *  leader archiviato dopo l'accesso viene fermato alla richiesta dopo. */
export function richiedeAuthTourLeader(req: Request, _res: Response, next: NextFunction) {
  ...
  tourLeaderAuthService.verificaAncoraAttivo(req.tourLeader.sub).then(() => next(), next);
}
