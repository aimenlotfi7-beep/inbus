...
export function OfferteTab({ eventoId, nomeEvento }: { eventoId: string; nomeEvento: string }) {
  ...
  function apriNuova() {
    ...
    setPrezzo(''); setPrezzoOriginale(''); setCampagnaId(''); setLimiteUtilizzi('');
    setFormAperto(true);
  }
  ...
  async function salva() {
    if (!nome.trim() || !slug.trim() || !prezzo) {
      alert('Compila almeno nome, link (slug) e prezzo.');
      return;
    }
    const payload: OffertaInput = {
      eventoId, nome, slug,
      prezzo: Number(prezzo),
      prezzoOriginale: prezzoOriginale ? Number(prezzoOriginale) : undefined,
  ...
  return (
    <div>
      <p className="testo-intro">
        Crea un link con un prezzo dedicato per una campagna pubblicitaria (es. "€40 invece di €50" per chi arriva
        da Meta). Il prezzo resta sempre nel database: il link pubblico porta solo un nome, mai il prezzo.
      </p>
      ...
      {formAperto && (
        <div className="section-card" style={{ marginBottom: 16 }}>
          ...
          <div className="form-grid">
            <label>Prezzo offerta (€) <input type="number" value={prezzo} onChange={(e) => setPrezzo(e.target.value)} /></label>
            <label>Prezzo originale, facoltativo (€) <input type="number" value={prezzoOriginale} onChange={(e) => setPrezzoOriginale(e.target.value)} placeholder="per mostrare 'invece di...'" /></label>
          </div>
          <div className="form-grid">
            <label>
      ...
      {offerte.map((o) => (
        <div key={o.id} className="riga-cliccabile" style={{ cursor: 'default', flexWrap: 'wrap' }}>
          <span className="riga-titolo">
            {o.nome} — €{Number(o.prezzo).toFixed(2)}{o.prezzoOriginale ? ` (invece di €${Number(o.prezzoOriginale).toFixed(2)})` : ''}
            <br />
