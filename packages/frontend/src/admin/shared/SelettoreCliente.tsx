    ...
    <div style={{ position: 'relative' }}>
      ...
      {aperto && ricerca.trim() && (
        <div style={{ position: 'absolute', zIndex: 5, top: '100%', left: 0, right: 0, background: 'var(--night)', border: '1px solid var(--line)', borderRadius: 8, maxHeight: 220, overflowY: 'auto', marginTop: 4 }}>
          {filtrati.length === 0 && <p style={{ padding: '10px 14px', fontSize: 13, color: 'var(--mist)' }}>Nessun cliente trovato.</p>}
          {filtrati.map((u) => (
            <button
              key={u.id}
              type="button"
              onClick={() => { onChange(u.id); setAperto(false); setRicerca(''); }}
              style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 14px', fontSize: 13.5, background: 'transparent', border: 'none', cursor: 'pointer' }}
            >
